...
static void IPCActionShutdown(void)
{
    ...
    if (reply.nb_sent != reply.nb_received) {
        Log().warning(ETIMEDOUT, "Shutdown req-response timed out for %d of %d apps",
                reply.nb_received, reply.nb_sent);
    }
}
...
static void SignalStop(int sig)
{
    switch (sig) {
        ...
        case SIGTERM:
            StopWorkers();
            if (LcoreStateCheckAllWTimeout(LCORE_RUNNING, 2) == 0) {
                IPCActionShutdown();
            } else {
                Log().debug("Stopping workers while the secondary app hasn't started yet");
                for (uint16_t i = 0; i < ctx.lcores_state.lcores_arr_len; i++) {
                    LcoreStateSet(ctx.lcores_state.lcores_arr[i].state, LCORE_WAIT);
                }
            }
