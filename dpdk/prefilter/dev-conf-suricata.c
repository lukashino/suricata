...
static int ConfGetDescendantValueInt(const ConfNode *base, const char *name, intmax_t *val)
{
    ...
    if (strval[0] == '\0' || *endptr != '\0') {
        SCLogError("malformed integer value for %s with base %s: '%s'", name, base->name, strval);
        return 0;
    }
    if (errno == ERANGE && (tmpint == INTMAX_MAX || tmpint == INTMAX_MIN)) {
        SCLogError(
                "integer value for %s with  base %s out of range: '%s'", name, base->name, strval);
        return 0;
    }
...
int DevConfSuricataStartRing(void *ring_conf)
{
    ...
    DPDKThreadVars ptv = {
        .port_id = re_suri->nic_conf.port1_id,
        .threads = re->sec_app_cores_cnt,
    };
...
static int DevConfSuricataStartPort(uint16_t pid, const char *pname, uint16_t rings_cnt)
{
    ...
    DevicePostStartPMDSpecificActions(&ptv, port_info.driver_name);
    return 0;
}
...
static int DevConfSuricataStopPort(uint16_t pid, const char *pname)
{
    ...
    DevicePreClosePMDSpecificActions(&ptv, port_info.driver_name);
    ret = rte_eth_dev_stop(pid);
    if (ret != 0) {
