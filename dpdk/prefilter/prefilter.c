...
static int IPCActionPktsStart(const struct rte_mp_msg *msg, const void *peer)
{
    ...
    if (ret == 0) {
        ...
        Log().warning(ENOTSUP, "Error (%s): Unable to reply for action %s", rte_strerror(rte_errno),
                IPC_ACTION_START);
        return -1;
    }
...
static int IPCActionBtDumpStart(const struct rte_mp_msg *msg, const void *peer)
{
    ...
    if (ret == 0) {
        ...
        Log().warning(ENOTSUP, "Error (%s): Unable to reply for action %s", rte_strerror(rte_errno),
                IPC_ACTION_BYPASS_TBL_DUMP_START);
        return -1;
    }
...
static int IPCActionBtDumpStop(const struct rte_mp_msg *msg, const void *peer)
{
    ...
    if (ret == 0) {
        ...
        Log().warning(ENOTSUP, "Error (%s): Unable to reply for action %s", rte_strerror(rte_errno),
                IPC_ACTION_BYPASS_TBL_DUMP_STOP);
        return -1;
    }
...
int main(int argc, char *argv[])
{
    ...
    ret = IPCInit(&ctx.status.actions, ctx.ring_conf_entries, ctx.ring_conf_entries_cnt);
    if (ret != 0)
        goto cleanup;
