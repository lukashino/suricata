...
int ParseTimeStringU64(const char *str, uint64_t *res)
{
     ...
     *  - Optional leading '+' accepted. Negative values rejected.
     *  - Unsigned integer only (no fractions to avoid ambiguity & overflow risk).
     *  - Optional unit token (letters only). Case-insensitive. No mixed digits/letters inside.
     *  - Entire string must be consumed (no trailing garbage).
     *  - On success: *res set to seconds; returns 0.
     *  - On invalid format/unit: return -2 (consistent with size parser invalid input logging style).
     *  - On overflow (base or base*multiplier): return -1.
     */
    if (res == NULL) {
        return -2;
    }
    *res = 0;
    if (str == NULL) {
        SCLogError("invalid time argument: NULL. Valid input is <number><unit> (e.g. 10s, 5m, 2h)");
        return -2;
    }
    const char *p = str;
    ...
    if (*p == '+') {
        ...
        return -2; /* negative not allowed */
    }
    if (*p == '\0') return -2; /* empty after spaces/sign */

    errno = 0;
    char *endptr = NULL;
    unsigned long long base = strtoull(p, &endptr, 10);
    if (errno == ERANGE) {
        return -1; /* overflow in numeric */
    }
    if (endptr == p) {
        return -2; /* no digits */
    }
    ...
    char unitbuf[16];
    size_t ulen = 0;
    const char *up = endptr;
    while (*up && isalpha((unsigned char)*up) && ulen + 1 < sizeof(unitbuf)) {
        unitbuf[ulen++] = (char)tolower((unsigned char)*up);
        up++;
    }
    unitbuf[ulen] = '\0';
    /* Any non-space trailing chars invalidate */
    while (isspace((unsigned char)*up)) up++;
    if (*up != '\0') {
        return -2; /* trailing garbage */
    }
    ...
    if (ulen != 0) {
        ...
        case 1:
            ...
            break;
        case 2:
            if (memcmp(unitbuf, "hr", 2) == 0) multiplier = 3600ULL; /* hr */
            else if (memcmp(unitbuf, "ms", 2) == 0) return -1; /* disallow milliseconds to avoid ambiguity */
            else return -1;
            break;
        case 3:
            if (memcmp(unitbuf, "sec", 3) == 0) multiplier = 1ULL;
            else if (memcmp(unitbuf, "min", 3) == 0) multiplier = 60ULL;
            else if (memcmp(unitbuf, "hrs", 3) == 0) multiplier = 3600ULL;
            else if (memcmp(unitbuf, "day", 3) == 0) multiplier = 86400ULL;
            else if (memcmp(unitbuf, "wks", 3) == 0) return -1; /* not supported abbreviation */
            else return -1;
            break;
        case 4:
            if (memcmp(unitbuf, "secs", 4) == 0) multiplier = 1ULL;
            else if (memcmp(unitbuf, "hour", 4) == 0) multiplier = 3600ULL;
            else if (memcmp(unitbuf, "days", 4) == 0) multiplier = 86400ULL;
            else if (memcmp(unitbuf, "year", 4) == 0) multiplier = 31536000ULL;
            else if (memcmp(unitbuf, "week", 4) == 0) multiplier = 604800ULL;
            else return -1;
            break;
        case 5:
            if (memcmp(unitbuf, "hours", 5) == 0) multiplier = 3600ULL;
            else if (memcmp(unitbuf, "weeks", 5) == 0) multiplier = 604800ULL;
            else if (memcmp(unitbuf, "years", 5) == 0) multiplier = 31536000ULL;
            else if (memcmp(unitbuf, "minute", 5) == 0) return -1; /* truncated */
            else return -1;
            break;
        case 6:
            if (memcmp(unitbuf, "minute", 6) == 0) multiplier = 60ULL;
            else if (memcmp(unitbuf, "second", 6) == 0) multiplier = 1ULL;
            else if (memcmp(unitbuf, "weekly", 6) == 0) return -1; /* not supported */
            else return -1;
            break;
        case 7:
            if (memcmp(unitbuf, "minutes", 7) == 0) multiplier = 60ULL;
            else if (memcmp(unitbuf, "seconds", 7) == 0) multiplier = 1ULL;
            else if (memcmp(unitbuf, "hourly", 7) == 0) return -1;
            else return -1;
            break;
        default:
            return -1; /* too long / unsupported */
        }
    }
...
static int UtilMiscParseTimeStringTest03(void)
{
    ...
    /* boundary: largest value that doesn't overflow with seconds (UINT64_MAX) truncated by overflow check */
