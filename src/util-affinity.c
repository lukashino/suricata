    ...
    TAILQ_FOREACH(lnode, &node->head, next) {
        ...
        if (!strcmp(lnode->val, "all")) {
            ...
            if (StringParseUint32(&a, 10, sep - lnode->val, lnode->val) < 0) {
                FatalError("%s: invalid cpu range (start invalid): \"%s\"", name, lnode->val);
#ifdef UNITTESTS
                return -1;
#endif
            }
            if (StringParseUint32(&b, 10, strlen(sep) - 1, sep + 1) < 0) {
                FatalError("%s: invalid cpu range (end invalid): \"%s\"", name, lnode->val);
#ifdef UNITTESTS
                return -1;
#endif
            }
            if (a > b) {
                FatalError("%s: invalid cpu range (bad order): \"%s\"", name, lnode->val);
#ifdef UNITTESTS
                return -1;
#endif
            }
            ...
            if (StringParseUint32(&a, 10, strlen(lnode->val), lnode->val) < 0) {
                FatalError("%s: invalid cpu range (not an integer): \"%s\"", name, lnode->val);
#ifdef UNITTESTS
                return -1;
#endif
            }
