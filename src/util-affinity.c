 ...
 * \param interface_name the affinity_set_name of the interface.
 * If NULL, the affinity is looked up by affinity_set_name only.
 *  \retval a pointer to the affinity or NULL if not found
 */
ThreadsAffinityType *AffinityTypeGetByIfaceOrCpuset(const char *affinity_set_name, const char *interface_name)
{
    ...
    for (int i = 0; i < MAX_CPU_SET; i++) {
        if (thread_affinity[i].name != NULL && strcmp(thread_affinity[i].name, affinity_set_name) == 0) {
...
static int ThreadingAffinityTest25(void)
{
    ...
    ThreadsAffinityType *child1 = AffinityTypeGetOrCreateByIfaceOrCpuset("worker-cpu-set", "iface1");
    ThreadsAffinityType *child2 = AffinityTypeGetOrCreateByIfaceOrCpuset("worker-cpu-set", "iface2");
    ThreadsAffinityType *child3 = AffinityTypeGetOrCreateByIfaceOrCpuset("worker-cpu-set", "iface3");
    ThreadsAffinityType *child4 = AffinityTypeGetOrCreateByIfaceOrCpuset("worker-cpu-set", "iface4");
