...
/** determine (without branching) if we include the pcap filename when comparing flows */
uint8_t g_pcap_filename_mask = 0xff;
    ...
    while ((opt = getopt_long(argc, argv, short_opts, long_opts, &option_index)) != -1) {
        switch (opt) {
            case 0:
...
#else
                    SCLogError("Libnet 1.1 support not enabled. Compile Suricata with libnet "
                               "support.");
                    return TM_ECODE_FAILED;
#endif
                        ...
                        for (int i = 0;; i++) {
                            if (suri->additional_configs[i] == NULL) {
                                const char **additional_configs = SCRealloc(
                                        suri->additional_configs, (i + 2) * sizeof(char *));
                                if (additional_configs == NULL) {
                                    FatalError("Failed to allocate memory for additional "
                                               "configuration "
                                               "files: %s",
                                            strerror(errno));
                                } else {
                ...
                } else if (strcmp((long_opts[option_index]).name, "firewall-rules-exclusive") ==
                           0) {
...
void SuricataInit(void)
{
    ...
    if (SCConfGetBool("pcap-filename.use-for-tracking", &tracking) == 1 && !tracking) {
        /* Ignore pcap filename when comparing flows. */
        g_pcap_filename_mask = 0x00;
    }
