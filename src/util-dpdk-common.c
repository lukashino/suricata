...
int DPDKDeviceResourcesInit(DPDKDeviceResources **dpdk_vars, uint16_t mp_cnt)
{
    ...
    *dpdk_vars = SCCalloc(1, sizeof(*dpdk_vars[0]));
    if (*dpdk_vars == NULL) {
        SCLogError("%s: failed to allocate memory for packet mempools structure", iconf->iface);
        SCReturnInt(-ENOMEM);
    }

    (*dpdk_vars)->pkt_mp = SCCalloc(iconf->nb_rx_queues, sizeof((*dpdk_vars)->pkt_mp[0]));
    if ((*dpdk_vars)->pkt_mp == NULL) {
        SCLogError("%s: failed to allocate memory for packet mempools", iconf->iface);
        SCReturnInt(-ENOMEM);
    }
    (*dpdk_vars)->pkt_mp_capa = iconf->nb_rx_queues;
