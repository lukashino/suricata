...
return CmpAddrsAndPorts(f_src, f_dst, f->sp, f->dp, p_src, p_dst, p->sp, p->dp) &&
       f->proto == p->proto && (f->recursion_level == p->recursion_level || g_recurlvl_mask == 0) &&
       CmpVlanIds(f->vlan_id, p->vlan_id) && (f->livedev == p->livedev || g_livedev_mask == 0) &&
       (g_pcap_filename_mask == 0 || ((f->pcap_filename[0] == '\0' && p_filename == NULL) ||
                                             (f->pcap_filename[0] != '\0' && p_filename != NULL &&
                                                     strcmp(f->pcap_filename, p_filename) == 0)));
}
