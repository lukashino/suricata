...
static void TmThreadJoinDpdkEalThread(ThreadVars *tv)
{
    int ret = rte_eal_wait_lcore(tv->lcore_id);
    if (ret < 0) {
        SCLogError(
            "%s: error waiting for DPDK lcore %"PRIu32" (%s) to finish (%s)", 
