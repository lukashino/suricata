        ...
        Packet *p, const void *pectx)
{
    ...
    if (p->flags & PKT_DETECT_HAS_STREAMDATA) {
        if (p->flags & PKT_PROFILE) {
            // HACK
            // the packet is reversed, we don't want to touch reassembly now
            return;
        }
        ...
        if (p->payload_len >= mpm_ctx->minlen) {
            ...
            if (det_ctx->mtc.save_stream_mpm_results && p->matched_sids_cnt + det_ctx->mtc.pids_count > MATCHED_SIDS_ARR_LEN_THRESH || det_ctx->mtc.pids[0] == UINT32_MAX) {
