...
void DpdkIpcDumpStats(void)
{
    ...
    retval = rte_mp_request_sync(&req, &reply, &ts);
    if (retval != 0 || reply.nb_sent != reply.nb_received) {
        FatalError("%s req-response failed (%s)", IPC_ACTION_BYPASS_TBL_DUMP_START,
        ...
        for (uint32_t idx = 0; idx < flow_config.hash_size; idx++) {
            ...
            while (f) {
                ...
                if (fc != NULL && fc->bypass_data != NULL) {
                    ...
                    if (ret != 0) {
                        SCLogDebug(
                                "Error (%s): Unable to enqueue message object", rte_strerror(-ret));
                        if (msg != NULL) {
                            msg->use_cnt--;
    ...
    if (retval != 0 || reply.nb_sent != reply.nb_received) {
        FatalError("%s req-response failed (%s)", IPC_ACTION_BYPASS_TBL_DUMP_STOP,
                rte_strerror(rte_errno));
    }
...
static uint16_t FlowKeyExtendedInitUnifiedIpv4(FlowKey *flow_key, struct FlowKeyDirection *fd,
        struct rte_ipv4_hdr *ip4_hdr, uint16_t *hdr_len)
{
...
static uint16_t FlowKeyExtendedInitUnifiedIpv6(FlowKey *flow_key, struct FlowKeyDirection *fd,
        struct rte_ipv6_hdr *ip6_hdr, uint16_t *hdr_len)
{
...
static void FlowKeyExtendedInitUnifiedTcp(
        FlowKey *flow_key, struct FlowKeyDirection *fd, struct rte_tcp_hdr *tcp_hdr)
{
...
static void FlowKeyExtendedInitUnifiedUdp(
        FlowKey *flow_key, struct FlowKeyDirection *fd, struct rte_udp_hdr *udp_hdr)
{
...
static void FlowBypassDuplicateMerger(Flow *f)
{
    ...
    Flow *f_dupl;
    while (f) {
        if (f != orig_f && CMP_ADDR(&f->src, &orig_f->src) && CMP_ADDR(&f->dst, &orig_f->dst) &&
                CMP_PORT(&f->sp, &orig_f->sp) && CMP_PORT(&f->dp, &orig_f->dp) &&
                f->proto == orig_f->proto &&
                ((f->vlan_id[0] ^ orig_f->vlan_id[0]) & g_vlan_mask) == 0 &&
                ((f->vlan_id[1] ^ orig_f->vlan_id[1]) & g_vlan_mask) == 0) {
