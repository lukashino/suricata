...
/** Verify truncated matches using explicit pattern IDs (from FPGA prefilter).
 *  Same logic as SCHSVerifyTruncatedMatches but uses a list of pattern indices
 *  instead of a bitset. */
        ...
        const char *ctx_type)
{
    ...
    for (uint32_t k = 0; k < pat_ids_cnt; k++) {
        uint32_t i = pat_ids[k];
        if (i >= pd->pattern_cnt)
            continue;
                    ...
                    PrefilterRuleStore *pmq, const uint8_t *buf, const uint32_t buflen)
{
    ...
    if (unlikely(buflen == 0)) {
        return 0;
    } else if (buflen >= (UINT32_MAX - MATCHED_SIDS_ARR_LEN_THRESH)) {
