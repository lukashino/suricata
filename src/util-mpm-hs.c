...
static int HSSaveCache(hs_database_t *hs_db, uint64_t hs_db_hash)
{
    ...
    int r = fwrite(db_stream, sizeof(db_stream[0]), db_size, db_cache_out);
...
static uint64_t SCHSHashDb(const PatternDatabase *pd)
{
...
static void HSSaveCacheIterator(void *data)
{
    ...
    if (HSSaveCache(pd->hs_db, SCHSHashDb(pd)) == 0) {
        pd->cache_loaded = true; // for rule reloads
    }
}
