...
int SCHSAddPatternCS(MpmCtx *, uint8_t *, uint16_t, uint16_t, uint16_t,
                     uint32_t, SigIntId, uint8_t);
int SCHSPreparePatterns(MpmCtx *mpm_ctx, const char *cache_dir_path);
...
int SCHSPreparePatterns(MpmCtx *mpm_ctx, const char *cache_dir_path)
{
    ...
    if (PatternDatabaseGetCached(&pd, cd, pd->no_cache ? NULL : cache_dir_path) == 0 &&
            pd != NULL) {
        ctx->pattern_db = pd;
 ...
 */
static int SCHSCacheRuleset(const char *dst)
{
    SCLogDebug("Caching the loaded ruleset to %s", dst);
    if (SCCreateDirectoryTree(dst, true) != 0) {
    ...
    struct HsIteratorData iter_data = { .pd_stats = &pd_stats, .cache_path = dst };
    SCMutexLock(&g_db_table_mutex);
...
static int SCHSTest01(void)
{
    ...
    SCHSPreparePatterns(&mpm_ctx, NULL);
...
static int SCHSTest11(void)
{
    ...
    if (SCHSPreparePatterns(&mpm_ctx, NULL) == -1)
        goto end;
