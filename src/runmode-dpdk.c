...
static void DPDKDerefConfig(void *conf)
{
    ...
    if (SC_ATOMIC_SUB(iconf->ref, 1) == 1) {
        DPDKDeviceResourcesDeinit(&iconf->dev_resources);
        SCFree(iconf);
    }
    SCReturn;
}
        ...
        const struct rte_eth_conf *port_conf)
{
    ...
    retval = DPDKDeviceResourcesInit(&(iconf->dev_resources), iconf->nb_rx_queues);
    if (retval < 0) {
        goto cleanup;
    }
    ...
    for (int i = 0; i < iconf->nb_rx_queues; i++) {
        ...
        iconf->dev_resources->pkt_mp[i] = rte_pktmbuf_pool_create(
                mempool_name, q_mp_sz, q_mp_cache_sz, 0, mbuf_size, (int)iconf->socket_id);
        if (iconf->dev_resources->pkt_mp[i] == NULL) {
            retval = -rte_errno;
    ...
    for (uint16_t queue_id = 0; queue_id < iconf->nb_rx_queues; queue_id++) {
        ...
        retval = rte_eth_rx_queue_setup(iconf->port_id, queue_id, iconf->nb_rx_desc,
                iconf->socket_id, &rxq_conf, iconf->dev_resources->pkt_mp[queue_id]);
        if (retval < 0) {
