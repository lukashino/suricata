        ...
        DPDKIfaceConfig *iconf, const char *entry_str, const struct rte_eth_dev_info *dev_info)
{
    ...
    if (entry_str == NULL || entry_str[0] == '\0' || strcmp(entry_str, "auto") == 0) {
        ...
        uint64_t mempool_size = (uint64_t)MempoolSizeCalculate(
                iconf->nb_rx_queues, iconf->nb_rx_desc, iconf->nb_tx_queues, iconf->nb_tx_desc);
        mempool_size = MempoolSizeDeviceAdjust(iconf->port_id, dev_info, (uint32_t)mempool_size);
        if (mempool_size > UINT32_MAX) {
            SCLogError("%s: required mempool size exceeds supported range", iconf->iface);
            SCReturnInt(-ERANGE);
        }
        iconf->mempool_size = (uint32_t)mempool_size;
        SCReturnInt(0);
    }
    ...
    uint64_t required_mp_size = (uint64_t)MempoolSizeCalculate(
            iconf->nb_rx_queues, iconf->nb_rx_desc, iconf->nb_tx_queues, iconf->nb_tx_desc);
    required_mp_size =
            MempoolSizeDeviceAdjust(iconf->port_id, dev_info, (uint32_t)required_mp_size);
    if (required_mp_size > UINT32_MAX) {
        SCLogError("%s: required mempool size exceeds supported range", iconf->iface);
        SCReturnInt(-ERANGE);
    }

    if ((uint32_t)required_mp_size >
        ...
        SCLogError("%s: mempool size is likely too small for the number of descriptors and queues, "
                ...
                iconf->iface, (uint32_t)required_mp_size);
        SCReturnInt(-ERANGE);
    }
