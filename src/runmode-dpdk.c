...
/**
 * \brief Input is a number of which we want to find the greatest divisor up to max_num (inclusive). The divisor is returned.
 */
...
/**
 * \brief Input is a number of which we want to find the greatest power of 2 up to num. The power of 2 is returned or 0 if no valid power of 2 is found.
 */
...
static uint32_t MempoolSizeCalculate(uint32_t rx_queues, uint32_t rx_desc, uint32_t tx_queues, uint32_t tx_desc)
{
...
static int ConfigSetMempoolSize(DPDKIfaceConfig *iconf, const char *entry_str)
{
    ...
    if (entry_str == NULL || entry_str[0] == '\0' || strcmp(entry_str, "auto") == 0) {
        ...
        if (iconf->nb_rx_desc == 0) {
            SCLogError("%s: cannot autocalculate mempool size without RX descriptors", iconf->iface);
            err = true;
        }
        ...
        iconf->mempool_size = MempoolSizeCalculate(iconf->nb_rx_queues, iconf->nb_rx_desc, iconf->nb_tx_queues, iconf->nb_tx_desc);
        SCReturnInt(0);
    }

    if (StringParseUint32(&iconf->mempool_size, 10, 0, entry_str) < 0) {
        SCLogError("%s: mempool size entry contain non-numerical characters - \"%s\"",
                iconf->iface, entry_str);
        SCReturnInt(-EINVAL);
    }

    if (MempoolSizeCalculate(iconf->nb_rx_queues, iconf->nb_rx_desc, iconf->nb_tx_queues, iconf->nb_tx_desc) > iconf->mempool_size + 1) { // +1 to mask mempool size advice given in Suricata 7.0.x - mp_size should be n = (2^q - 1)
        SCLogError("%s: mempool size is likely too small for the number of descriptors and queues, set to \"auto\" or adjust to the value of \"%" PRIu32 "\"", iconf->iface, MempoolSizeCalculate(iconf->nb_rx_queues, iconf->nb_rx_desc, iconf->nb_tx_queues, iconf->nb_tx_desc));
        SCReturnInt(-ERANGE);
    }
...
static int ConfigSetTxDescriptors(DPDKIfaceConfig *iconf, const char *entry_str, uint16_t max_desc)
{
    SCEnter();
    if (entry_str == NULL || entry_str[0] == '\0') {
        SCLogInfo("%s: number of TX descriptors not found, going with: %s", iconf->iface, DPDK_CONFIG_DEFAULT_TX_DESCRIPTORS);
...
static int ConfigLoad(DPDKIfaceConfig *iconf, const char *iface)
{
    ...
    if (retval < 0) {
        SCLogError("%s: too many threads configured - reduce thread count to: %" PRIu16, iconf->iface, dev_info.max_rx_queues);
        SCReturnInt(retval);
    }
    ...
    if (retval < 0) {
        SCLogError("%s: too many threads configured - reduce thread count to: %" PRIu16, iconf->iface, dev_info.max_tx_queues);
        SCReturnInt(retval);
    }

    retval = ConfGetChildValueWithDefault(
                     if_root, if_default, dpdk_yaml.rx_descriptors, &entry_str) != 1
                     ? ConfigSetRxDescriptors(iconf, DPDK_CONFIG_DEFAULT_RX_DESCRIPTORS, dev_info.rx_desc_lim.nb_max)
    ...
    retval = ConfGetChildValueWithDefault(
                     ...
                     ? ConfigSetTxDescriptors(iconf, DPDK_CONFIG_DEFAULT_TX_DESCRIPTORS, dev_info.tx_desc_lim.nb_max)
        ...
        const struct rte_eth_conf *port_conf)
{
    ...
    SCLogInfo("%s: creating %u packet mempools of size %u, cache size %u, mbuf size %u", iconf->iface, iconf->nb_rx_queues, q_mp_sz, q_mp_cache_sz, mbuf_size);
    for (int i = 0; i < iconf->nb_rx_queues; i++) {
        char mempool_name[64];
        snprintf(mempool_name, sizeof(mempool_name), "mp_%d_%.20s", i, iconf->iface);
        iconf->pkt_mempools->pkt_mp[i] = rte_pktmbuf_pool_create(mempool_name, q_mp_sz,
