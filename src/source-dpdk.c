...
static bool InterruptsRXEnable(uint16_t portid, uint16_t queueid)
{
    ...
    if (ret != 0) {
        SCLogError("%s-Q%d: failed to enable interrupt mode: %s", DPDKGetPortNameByPortID(portid),
                queueid, rte_strerror(-ret));
        return false;
    }

    SCLogDebug(
            "%s-Q%d interrupt mode enabled", DPDKGetPortNameByPortID(ptv->port_id), ptv->queue_id);
    return true;
}
...
static TmEcode ReceiveDPDKLoop(ThreadVars *tv, void *data, void *slot)
{
    ...
    if (ptv->intr_en && !InterruptsRXEnable(ptv->port_id, ptv->queue_id))
